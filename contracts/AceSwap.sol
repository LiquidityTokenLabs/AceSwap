...
//EVENT
  event StakeNFT(address indexed staker, uint256[] tokenIds, uint256 LP, uint256 eventId);
  event SwapNFTforFT(address indexed swapper, uint256[] tokenIds, uint256 totalFee, uint256 eventId);
  event SwapFTforNFT(address indexed swapper, uint256[] tokenIds, uint256 totalFee, uint256 eventId);
  event RemoveLP(address indexed staker, uint256[] tokenIds, uint256 removeLP, uint256 eventId);
...
//CONSTRUCTOR
  ...
  function stakeNFT(uint256[] memory tokenIds) public {
    ...
    unchecked {
      for(uint256 i = 0; i < _tmpNum; i++){
        require(IERC721(collectionAddress).ownerOf(tokenIds[i]) == msg.sender,"you are not owner");
      }
    }
    ...
    unchecked {
      for(uint256 i = 0; i < _tmpNum; i++){
        IERC721(collectionAddress).safeTransferFrom(msg.sender,address(this),tokenIds[i],"");
      }
    }
  ...
  function swapFTforNFT(uint256[] memory tokenIds) public payable {
    uint256 _tmpNum = tokenIds.length;
    (uint256 _totalFee, uint256 _newSpotPrice) = _calcBuyInfo(_tmpNum, spotPrice, delta);
    ...
    unchecked {
      for(uint256 i = 0; i < _tmpNum; i++){
        require(IERC721(collectionAddress).ownerOf(tokenIds[i]) == address(this));
      }
    }
    ...
    unchecked {
      for(uint256 i = 0; i < tokenIds.length; i++){
        IERC721(collectionAddress).safeTransferFrom(address(this), msg.sender, tokenIds[i], "");
      }
    }
  ...
  function swapNFTforFT(uint256[] memory tokenIds,uint256 minExpectFee) public payable {
    uint256 _tmpNum = tokenIds.length;
    (uint256 _totalFee, uint256 _newSpotPrice) = _calcSellInfo(_tmpNum, spotPrice, delta);
    ...
    require(_totalFee >= minExpectFee, "The amount you get is lower than expected.");
...
//CALCULATION
  ...
  function _calcLP(uint256 _num)internal view returns(uint256){
    ...
    if(totalLPoint == 0){
      ...
      _LP = totalLPoint * (_num * spotPrice) / (initBuyNum * spotPrice + totalFee);
    }
  ...
  function _calcRemoveLP(uint256 _num, address _user)internal view returns(uint256 _userFee){
    uint256 _tmpLP = (initBuyNum * spotPrice + totalFee) * userLPoint[_user] / totalLPoint;
  ...
  function _calcBuyInfo (
    ...
    totalFee = _numItems * _spotPrice + (_numItems * (_numItems - 1) * _delta) / 2;
  ...
  function _calcSellInfo (
    ...
    totalFee = _numItems * (_spotPrice - _delta * 2  ) - (_numItems * (_numItems - 1) * _delta) / 2;
...
//GET
  ...
  function getCalcBuyInfo(uint256 _numItems)external view returns(uint256 totalFee){
    totalFee = _numItems * spotPrice + (_numItems * (_numItems - 1) * delta) / 2;
    console.log(totalFee);
  }

  //@notice get total sell price
  function getCalcSellInfo(uint256 _numItems)external view returns(uint256 totalFee){
    totalFee = _numItems * (spotPrice - delta * 2  ) - (_numItems * (_numItems - 1) * delta) / 2;
    console.log(totalFee);
  }
